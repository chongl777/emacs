...
function updateContent() {
    var t_date = d3.time.format('%Y-%m-%dT%H:%M').parse(
        paramsInput.t_date())
    var ref_date = d3.time.format('%Y-%m-%dT%H:%M').parse(
...
var paramsInput = new (function () {
    ...
    this.t_date = ko.observable(d3.time.format("%Y-%m-%dT%H:%M")(t_date));
    this.ref_date = ko.observable(d3.time.format("%Y-%m-%dT%H:%M")(ref_date));
