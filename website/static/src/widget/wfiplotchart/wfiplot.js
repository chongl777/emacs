(function($){
    $.fn.init_wfi_plot = function(options) {
        var self = this;
        ...
        self.updateWithData = function(data, args) {
            self.each(function(i, div) {
                $('div', div)[0].updateWithData(data, args);
            })
        };
    ...
    function add_main_axis(self, data) {
        ...
        self.options.x_axis = d3.svg.axis()
                .scale(self.options.xaxis.scale)
                .orient("bottom").ticks(5)
                .innerTickSize(-self.options.height)
                .outerTickSize(0)
        ...
        var y1_axis = d3.svg.axis()
                ...
                .innerTickSize(self.options.width)
                .outerTickSize(0);
    ...
    function add_main_panel(self) {
        ...
        d3.select(self).select("g#plot-area")
            ...
            .append('rect')
            .attr({width: self.options.width, height: self.options.height})
            .attr('z-index', 10);
        ...
        var plotClip = d3.select(self).select("g#plot-area")
                ...
                .attr({width: self.options.width, height: self.options.height}).attr('opacity', 0);
    }
    ...
    function add_navi_chart(self, data) {
        ...
        var navXAxis = d3.svg.axis()
                ...
                .orient('bottom')
                .ticks(5)
                .innerTickSize(-self.options.naviHeight)
                .outerTickSize(-self.options.naviHeight)
        ...
        var brush = d3.svg.brush()
                .x(navXScale)
                .on("brush", function() {brushed.call(self, data, brush, true)});
        brush.clear();
        // self.options.navigator.default &&
        //     brush.extent(self.options.navigator.default(navi.slice()));
        ...
        function drawBrush(start, end) {
            ...
            brush.event(d3.select("g.x.brush"));
        }
    }
    ...
    function brushed(data, brush) {
        ...
        // self.options.x.domain(brush.empty()? self.options.navXScale.domain() : brush.extent());
        var [start, end] = brush.empty()? self.options.navigator.scale.domain() : brush.extent();
        // adjust start, end
        var pos_i, pos_j;
    ...
    function DataTransform(data, index, start, end) {
        var data_adj = [];
        var yDomain = [];
        $.each(data, function(p, i) {
            var d = normData(data[p], index, start, end);
