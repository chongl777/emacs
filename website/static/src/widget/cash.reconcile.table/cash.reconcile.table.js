(function($){
    ...
    function add_table(self) {
        ...
        var header = ['Currency', 'Net_Amount_GS', 'Net_Amount_WFI', 'Diff'];
        ...
        function add_child_table_trades(data, depth) {
            ...
            var header = [
                'Trade ID', 'Trade Date', 'SecurityID', 'Description', 'SecType',
                'Quantity(GS)', 'Quantity(WFI)', 'Quantity Diff',
                'Net Amount(GS)', 'Net Amount(WFI)', 'Net Amount Diff', ''];
            header.forEach(function(p, i) {
                var tr = $("<th>" + p + "</th>");
                $("tr", html).append(tr);
            });
            ...
            var columns_setup = [
                ...
                {data: "trade_dt",
                 ...
                 render: function(value) {
                     try {
                         if (value == "" | value == null) return '<div class="value"></div>';
                         return '<div class="value">' + d3.time.format("%Y-%m-%d")(value) + '</div>';
                     } catch(err) {
                         return '<div class="value"></div>'
                     }
                 }},
        ...
        function add_child_table_other_cash(data, depth, create_table_gs, create_table_wfi) {
            ...
            var header = [
                'Trade Date', 'Net Amount(GS)', 'Net Amount(WFI)', 'Net Amount Diff', ''];
            header.forEach(function(p, i) {
                var tr = $("<th>" + p + "</th>");
                $("tr", html).append(tr);
            });
        ...
        function add_child_table_unclassified(data, depth) {
            ...
            var header = [
                'Trade Date', 'Category', 'Net Amount(GS)', 'Net Amount(WFI)', 'Net Amount Diff', ''];
            header.forEach(function(p, i) {
                var tr = $("<th>" + p + "</th>");
                $("tr", html).append(tr);
            });
    ...
    function preProcess(data) {
        ...
        data.forEach(function(p) {
            try {
                p['trade_dt'] = d3.time.format('%Y-%m-%dT%H:%M:%S').parse(p['trade_dt']);
            } catch(err) {
            };
            dataClone.push(p);});
    ...
    function show_detailed_records(tbl_gs, tbl_wfi) {
        ...
        overlay.html('<div class="wfidatatable args-dlg 1tgt-px-dlg" style="width: 1200px; height:auto;"></div>');
    ...
    function create_table_wfi_other_cash(data, callback) {
        ...
        var header = [
            'Trade ID', 'Trade Dt', 'Currency', 'Account', 'Comments', 'Net Amount', 'FX Rate'];
        header.forEach(function(p, i) {
            var tr = $("<th>" + p + "</th>");
            $("tr", html).append(tr);
        });
