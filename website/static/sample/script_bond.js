...
function updateContent(submit) {
    var t_date = d3.time.format('%Y-%m-%d').parse(
        paramsInput.t_date())
    var s_date = d3.time.format('%Y-%m-%d').parse(
        paramsInput.s_date())
...
var paramsInput = new (function () {
    ...
    this.t_date = ko.observable(d3.time.format("%Y-%m-%d")(t_date));
    this.s_date = ko.observable(d3.time.format("%Y-%m-%d")(s_date));
...
var create_screening_table;

function initialize_bond(PARAMS) {
    PARAMS['widget']['security-screening'] = addTitle(
        create_screening_table(
            query='div#security-screening',
